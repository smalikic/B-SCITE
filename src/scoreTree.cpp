...
// SALEM: Code below now needs revision because we are not moving anymore solely in the SCITE + CITUP search space
//	double approxScore = scoreTreeFast(n, m, logScores, dataMatrix, type, parentVector);

//	if(approxScore > bestTreeLogScore-epsilon)
//	{
		return scoreTreeAccurate(n, m, logScores, dataMatrix, type, parentVector);   // than the current best score, use accurate
//	}                                                                                // score computation

//	return approxLogScore;                                                              // otherwise the approximate score is sufficient
}
...
double** getLogScores(double FD, double AD1, double AD2, double CC){
  ...
  logScores[0][0] = log(1.0-CC-FD);  // observed 0, true 0
	...
	if(CC!=0.0){
		logScores[2][0] = log(CC);         // observed 2, true 0   SALEM: 2 stands for homozygous mutation
	}
